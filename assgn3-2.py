...
def pAzpx(xa,ya,xb,yb,nkn=[1,1]):
    ...
    px2 = nkn[1] * (-px1_)
    py2 = nkn[1] * (-py1_)
    return [px1 * RHOSEC/TO_M,py1 * RHOSEC/TO_M,px2*RHOSEC/TO_M,py2*RHOSEC/TO_M]
...
def powerEdge(dist,sig):
    return (sig/(1 + 1e-6 * dist))**2   #if the empirical eq.'s unit is cm
...
def helmertEst(n1:int,n2:int,nt:int,mb:np.matrix,mp:np.matrix,xhat,lmin):
    ...
    '''
    elementa not at diag my be negative, cause result sigma0 to be negative when the level of
...
if __name__ == "__main__":
    ...
    for itnl in range(5):  # iteration for non linear
        ...
        for itvc in range(0,30):
            the1 = sig0beta
            the2 = sig0*10  #empirical
            the12 = the1
            ...
            mw = np.matrix([\
                ...
                getAng(ptD, ptB, ptA),\
                getAng(ptD, ptA, ptC),\
                TO_M*getDistp(ptB,ptD),\
                TO_M*getDistp(ptC,ptD),\
                TO_M*getDistp(ptC,ptA),\
                TO_M*getDistp(ptA,ptD),\
                TO_M*getDistp(ptC,ptB),\
                ]).T
            ...
            #variation component estimate
            thetas = helmertEst(8, 5, 4, mb, mp, dx, mlmaj - mw)
            sig0beta = sig0beta / thetas[0,0]
            sig0 = sig0 / thetas[1,0]
            if abs(sig0beta-1.0)<1e-5 and abs(sig0-1.0)<1e-5:
                print(f"iteration for variation component ends at{itvc+1}")
                break
    ...
    print("whatever, the last result is")
    print(mx)
